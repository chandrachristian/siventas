...
class InventarisController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $inventaris = new InventarisModel([
            ...
            "kode_inventaris" => "J" . $jenis->kode_jenis . "-R" . $ruang->kode_ruang . "-P" . \Auth::user()->petugas->id . "-B" . random_int(0, 1000),
        ...
        if ($inventaris->save()) {
            return redirect()
                ...
                ->with('success', 'Inventaris berhasil ditambah');
        } else {
            return redirect()
                ->route('inventaris.index')
                ->with('error', 'Inventaris gagal ditambah');
        }
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($inventaris->save()) {
            return redirect()
                ->route('inventaris.index')
                ->with('success', 'Inventaris berhasil diubah');
        } else {
            return redirect()
                ->route('inventaris.index')
                ->with('error', 'Inventaris gagal dubah');
        }
    }
    ...
    public function delete($id)
    {
        ...
        if ($inventaris->delete()) {
            ...
            return redirect()
                ->route('inventaris.index')
                ->with('error', 'Inventaris gagal dihapus');
        }
    }
}
