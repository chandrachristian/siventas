...
class PegawaiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($user->save()) {
            ...
            if ($pegawai->save()) {
                ...
                return redirect()
                    ->route('pegawai.index')
                    ->with('success', 'Member berhasil ditambah');
            } else {
                DB::rollBack();
                return redirect()
                    ->route('pegawai.index')
                    ->with('error', 'Member gagal ditambah');
            }
        } else {
            DB::rollBack();
            return redirect()
    ...
    public function delete($id)
    {
        ...
        if ($pegawai->delete() && $user->delete()) {
            ...
            return redirect()
                ->route('pegawai.index')
                ->with('error', 'Member gagal dihapus');
        }
    }
    ...
    public function switchStatusAccount(Request $r, $id)
    {
        ...
        return redirect()->route('pegawai.index')
            ->with('success', 'Status Member diubah menjadi "' . $messages . '"');
    }
}
